...
NumPy中的基本对象是**同类型**的的多维数组,字符型和数值型就不可共存于同一个数组中.

> 生成一个一维数组a，从0开始，步长为1，长度为20
...
既然a是**array**，便可以调用array的函数进一步查看a的相关属性
